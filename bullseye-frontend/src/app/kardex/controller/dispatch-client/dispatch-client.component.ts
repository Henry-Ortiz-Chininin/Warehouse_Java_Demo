...
@Component({
  selector: 'app-dispatch-client',
  templateUrl: './dispatch-client.component.html',
  styleUrls: ['./dispatch-client.component.css']
})
export class DispatchClientComponent implements OnInit {


  today = new Date();
  pipe = new DatePipe('en-US');
  dispatch: ClientDispatch = new ClientDispatch();
  ...
  ngOnInit(): void {
    ...
    this.dispatch.NewProduct.Quantity=1;
  ...
  onChangeProduct(){
    ...
    this.dispatch.NewProduct.UnitPrice=this.productSelected.unitPrice;
    this.dispatch.NewProduct.TotalPrice=this.dispatch.NewProduct.UnitPrice * this.dispatch.NewProduct.Quantity;
  };
  ...
  getNewReception(){
    ...
    this.dispatchService.getNewReception().subscribe(data => {
      ...
      this.warehousesOUT = this.warehouses.filter(e=>e.Type=="STR");
    });

  }

  save(){
    this.dispatchService.createClient(this.dispatch).subscribe(data => {
      if(data.Status =="OK"){
